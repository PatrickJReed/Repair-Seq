...
Gerp_in_Peaks <- read.delim("~/RepairedSeq/Gerp_in_Peaks.bed", header=FALSE)
RepairPeaks_By_Gerp <-  split(Gerp_in_Peaks,Gerp_in_Peaks$V16)

Gerp_by_Peak <-  data.frame(matrix(vector(), 37549, 2,dimnames=list(c(), c("Peak_Max_Gerp","Conc"))),stringsAsFactors=F)

for (i in 1:37549) {
  Gerp_by_Peak$Peak_Max_Gerp[i] <- max(RepairPeaks_By_Gerp[[i]]$V21)
  Gerp_by_Peak$Conc[i] <- max(RepairPeaks_By_Gerp[[i]]$V6)
}
...
```{r}
## B
## DNA shape analysis predictions for sequences found in genome repair hotspots.
...
```{r}
## C
## Histograms of distance from genome repair hotspots to H3K27Ac, ATAC, H3K27Ac & ATAC, H3K9me3, or CTCF peaks.
...
```{r}
## D
## Scatter plots: Axis: H3K27Ac/H3K9me3 or H3K27Ac/ATAC points/intensity=salmon repair classes & genome repair hotspot groups
...
```{r}
## F
## Histograms of pairwise distance (CTCF, ATAC, H3K9me3, H3K27Ac)
...
```{r}
## Other
## Box plots of Salmon Repair/Peaks Repair vs gene TPMs for H3K27Ac & H3K9me3



## Motifs on EdC vs EdU vs EdC+EdU



## % GC of peaks (EdU vs EdC)



##  Pqsfinder by genome annotation
```
