...
ggplot(H9_subset_df, aes(x=H9, y=H9.1)) + geom_point(size=.2) + theme_classic() + theme(panel.border = element_rect(colour = "black", fill=NA, size=.5)) + stat_poly_eq(formula = my.formula, aes(label = paste(..eq.label.., ..rr.label.., sep = "~~~")), parse = TRUE) + xlab("H9 ESC-iN Replicate 1") + ylab("H9 ESC-iN Replicate 2") + xlim(0,500) + ylim(0,500)
#dev.off()

#pdf("H1_Replicate_Correlation.pdf")
my.formula <- y ~ x
ggplot(H1_subset_df, aes(x=H1, y=H1.1)) + geom_point(size=.2) + theme_classic() + theme(panel.border = element_rect(colour = "black", fill=NA, size=.5)) + stat_poly_eq(formula = my.formula, aes(label = paste(..eq.label.., ..rr.label.., sep = "~~~")), parse = TRUE) + xlab("H1 ESC-iN Replicate 1") + ylab("H1 ESC-iN Replicate 2") + xlim(0,500) + ylim(0,500)
#dev.off()
